...
class LoginService {
  ...
  async onPhaseChanged(event) {
    ...
    if (configs.COMPRESSION_THRESHOLD >= 0) {
      await this.enableCompression(channel, protocolVersion, configs.COMPRESSION_THRESHOLD);
    }
  ...
  async startLogin(channel) {
    ...
    const uuid = generateOfflineUuid(loginStartPacket.playerName);
  ...
  async enableCompression(channel, protocolVersion, threshold) {
